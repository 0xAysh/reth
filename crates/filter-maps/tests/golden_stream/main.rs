...
//! only compress consecutive identical values; every index is expanded and compared. H/B describe
//! Rust completion contracts using Geth's observed pointer/cursor, not native Geth event types.
...
macro_rules! fixture_tests {
    ($($name:ident => $file:literal),+ $(,)?) => { $(
        #[test]
        fn $name() { check(include_str!(concat!("fixtures/", $file, ".txt"))); }
    )+ };
}
...
fn actual_batch_completion_drives_continuation_fixture() {
    let first = parse(include_str!("fixtures/batch-before-padding.txt"));
    ...
    let next = parse(include_str!("fixtures/continuation-padding.txt"));
    let actual =
